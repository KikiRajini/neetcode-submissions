...
//We don't use < or > because permutations require exact frequency matching (==), whereas inequality operators cannot detect when a count overshoots or undershoots the target bucket size. Because < and > evaluate true or false across multiple values (e.g., 2>1, 3>1, 4>1 are all true), they cannot distinguish between a state change that breaks a match versus a state change that keeps it mismatched.
//1→2 is the boundary transition where a match breaks (ON to OFF), whereas 2→3 is a redundant invalid state (OFF stays OFF) that requires no change to matches.
